...
initi2c()
{
  ...
  //Pinmux I2C_SCLK and I2C_SDA
	// pin mux SCL:mux_mode 2     SDA: mux_mod 2
	PinMuxing(CONF_I2C_SCL,PULL_DISABLE,0,2); //initialise pin SCL
	PinMuxing(CONF_I2C_SDA,PULL_DISABLE,0,2); //initialise pin SCL
  ...
  //Disable I2C Module for Configuration
	HWREG ( SOC_I2C_1_REGS + I2C_CON ) &= ~(1 << 15); // i2c deaktivieren

  //Prescale functional clock to internal i2c clock
	HWREG ( SOC_I2C_1_REGS + I2C_PSC ) |= ((1 << 1) | (1 << 0)); // prescaler für Clocksignal einstellen,here we set to 4

  //Comparematch for low-time
	HWREG ( SOC_I2C_1_REGS + I2C_SCLL ) |= ((1 << 3) | (0 << 2)| (0 << 1) | (0 << 0)); // geschwendigkeit setzen, here scll 8

  //Comparematch for high-time
	HWREG ( SOC_I2C_1_REGS + I2C_SCLH ) |= ((1 << 3) | (0 << 2)| (1 << 1) | (0 << 0)); // geschwendigkeit setzen, here sclh 10

  //Enable I2C Module again
	HWREG ( SOC_I2C_1_REGS + I2C_CON ) |= (1 << 15); // i2c aktivieren
...
readfromi2c(unsigned int sla_address, char*data, unsigned char count, char stp)
{
  ...
  //configure I2C controller in Master Receiver mode
	HWREG(SOC_I2C_1_REGS + I2C_CON) &=~ (1<<9); //page 183
	HWREG(SOC_I2C_1_REGS + I2C_CON) = I2C_CON_MST | I2C_CON_I2C_EN;//!!!!!

  //enable receive interrupt
	HWREG(SOC_I2C_1_REGS + I2C_IRQENABLE_SET) |= I2C_IRQSTATUS_RRDY;//!!!!!!!!!! (1<<3)
  ...
  //disable receive interrupt
   HWREG(SOC_I2C_1_REGS + I2C_IRQENABLE_CLR) = I2C_IRQSTATUS_RRDY; //_SET write 0 no effect!! !!!!!! (1<<3)
